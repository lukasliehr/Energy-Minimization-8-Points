...
open Thomson8 (quadratic quadratic_nonneg)
...
theorem sos_value (b : Block) (w : Fin 192 → ℝ) (hm : b.moment=false) (u v t : ℝ) :
    b.value w u v t = Poly.symEval (fun u v t => (if b.weighted then 1-u^2 else 1)*
      quadratic (b.matrixFrom w) (b.polyVector u v t)) u v t := by
  unfold value
  simp_rw [sos_column_eval b hm]
  have hf (i : Fin b.n) :
      (∑ j : Fin b.n, (b.matrixFrom w) i j * Poly.symEval (fun u v t =>
        (if b.weighted then 1-u^2 else 1)*
          ((b.polys.getD i []).eval u v t*(b.polys.getD j []).eval u v t)) u v t) =
      Poly.symEval (fun u v t => ∑ j : Fin b.n, (b.matrixFrom w) i j *
        ((if b.weighted then 1-u^2 else 1)*
          ((b.polys.getD i []).eval u v t*(b.polys.getD j []).eval u v t))) u v t := by
...
theorem sos_value_nonneg (b : Block) (w : Fin 192 → ℝ) (hm : b.moment=false) (hb : (b.matrixFrom w).PosSemidef)
    (u v t : ℝ) (hu : -1 ≤ u ∧ u ≤ 1) (hv : -1 ≤ v ∧ v ≤ 1) (ht : -1 ≤ t ∧ t ≤ 1) :
    0 ≤ b.value w u v t := by
  rw [sos_value b w hm]
  have h (u v t : ℝ) (hu : -1 ≤ u ∧ u ≤ 1) :
      0 ≤ (if b.weighted then 1-u^2 else 1)*quadratic (b.matrixFrom w) (b.polyVector u v t) := by
    apply mul_nonneg _ (quadratic_nonneg (b.matrixFrom w) hb _)
