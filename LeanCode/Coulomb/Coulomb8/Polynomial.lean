...
def isZero (p : Poly) : Prop := ∀ m ∈ p, K.isZero m.2
instance (p : Poly) : Decidable (isZero p) := inferInstanceAs (Decidable (∀ m ∈ p, K.isZero m.2))
...
theorem eval_of_isZero (p : Poly) (h : p.isZero) (u v t : ℝ) : eval p u v t = 0 := by
  ...
  rw [K.eval_of_isZero _ (h m hm), zero_mul]
